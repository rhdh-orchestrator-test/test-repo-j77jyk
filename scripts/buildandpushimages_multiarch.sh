...
echo 'building and pushing image test-repo-j77jyk-buildstage'
cd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/test-repo-j77jyk-buildstage .
cd -

echo 'building and pushing image test-repo-j77jyk'
cd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/test-repo-j77jyk .
cd -
