...
## Running test-repo-j77jyk in Quarkus dev mode
...
```
(test-repo-j77jyk should be included in the returned array)
...
[test-repo-j77jyk-schema.json](./src/main/resources/schemas/test-repo-j77jyk-schema.json ), run the following to create an
instance of the test-repo-j77jyk workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/test-repo-j77jyk -d@input.json
```
